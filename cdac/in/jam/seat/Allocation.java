...
=======
    ...
    boolean allocation(String paper, List<Applicant> applicants, Map<String, Course> courses){
        ...
        for(Applicant applicant: applicants){
            ...
            if( applicant.statusId == 5 ){	                                                /* Candidate who has not accepted the offer */
                continue;
            }
            ...
            for( int choiceNo = 0; choiceNo < applicant.validChoices.length && choiceNo <= applicant.allocatedChoice; choiceNo++){
                ...
                if( course != null ){
                    ...
                    if( choiceNo == applicant.allocatedChoice ){                            /* same-choice  */
                        ...
                        if( applicant.allocatedQuota.name.equals("GEN") ){                  /* All ready have the GEN Seat */
                            break;    
                        }

                        if( allocate("GEN", course, applicant, choiceNo, freedSeats ) ){    /* from Category/PwD seat to GEN Seat */
                            flag = true;
                            break;    
                        }    
                    ...
                    if( course != null ){
                        ...
                        if( quota != null){
                            ...
                            while( quota.closingRank == applicant.ranks.get(paper).rank ){
                                ...
                                Applicant app =  quota.allocatedCandidate.get( quota.allocatedCandidate.size() - 1 );    

                                if( app.lastroundseat )
                                    break;      

                                 quota.allocatedCandidate.remove( quota.allocatedCandidate.size() - 1); 

                                 System.out.println("# "+app.applicationId+", "+app.allocatedQuota.name+", "+app.allocatedQuota.paper+", "+app.allocatedQuota.programCode+", "+app.ranks.get( app.allocatedQuota.paper ).rank+", "+app.lastroundseat );				
